 ...
 *
 * Usage:
 * block-bat perc    battery name (BAT0) NULL on OpenBSD/FreeBSD
 *           state   battery name (BAT0) NULL on OpenBSD/FreeBSD
 *           rem     battery name (BAT0) NULL on OpenBSD/FreeBSD
 *
 *
 */
...
int main(int argc, char **args) {
  ...
  if (argc == 3) {
    switch (args[1][0]) {
    case 'p':
      printf("%s", battery_perc(args[2]));
      break;
    case 's':
      printf("%s", battery_state(args[2]));
      break;
    case 'r':
      printf("%s", battery_remaining(args[2]));
      break;
    }
  }
  return 0;
